...
- (CTFrameRef)frameRefFromSize:(CGSize)size textRectOutput:(CGRect *)textRectOutput CF_RETURNS_RETAINED {
  ...
  CGRect contentRect = [self contentRectFromSize:self.bounds.size withInsets:self.textInsets];

  UIFont *fontToDraw = self.font;
  if (self.adjustsFontSizeToFitWidth) {
    BOOL textFits = NO;
    do {
      NSStringDrawingOptions options = (self.numberOfLines == 1) ? NSStringDrawingUsesFontLeading :
      (NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin);
      CGSize sizeNeeded = [self.text boundingRectWithSize:CGSizeMake(INT_MAX, INT_MAX)
                                                  options:options
                                               attributes:@{NSFontAttributeName : [UIFont fontWithName:fontToDraw.fontName size:fontToDraw.pointSize]}
                                                  context:nil].size;
      textFits = (sizeNeeded.height < contentRect.size.height ) && (sizeNeeded.width < contentRect.size.width);

      if (!textFits) {
        fontToDraw = [UIFont fontWithName:fontToDraw.fontName size:fontToDraw.pointSize - 1];
      }
    } while (!textFits && fontToDraw.pointSize > 0);
  }


  CTFontRef fontRef = CTFontCreateWithName((__bridge CFStringRef)fontToDraw.fontName, fontToDraw.pointSize, NULL);
